...
## 下一个版本
**bug修复**
- [x] 退出指令的函数异常导致长时间工作崩溃的bug
- [x] 使用指令后会@自己导致无止境自我回复的bug
- [x] 管理员获取不全的问题(最大化窗口读取去修复)
- [x] 最高权限者和机器人同名无法调用限制指令的bug
- [x] 修复窗口重置时间无效
 
**使用优化**
- [x] 优化`#帮助`指令的内容
- [x] `#人设切换`指令自动清空对话历史
- [x] 爱丽丝Pro去除涩涩限制（修改了人设库）
- [x] 优化`#退出`指令使用后更加优雅的退出该线程而不是整个软件。

**增加功能**
- [x] 软件界面截图发送(监控软件状态)
- [x] 增加获取软件运行时间指令(监控软件运行时间，稳定性)
- [x] QQ消息轮询时间自定义(如：每几秒检查一次QQ新消息)
