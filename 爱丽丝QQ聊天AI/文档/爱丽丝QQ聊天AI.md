...
1. 打开并登陆QQ，设置中在点击“超级调色盘”把主题选择为“极简。（登陆的账号新手强烈建议使用小号，这个账号将作为机器人账号使用）【在本软件2.1.0版本后不需要了，对QQ实现了调色板主题实现了兼容(进行了穷尽测试)！！！】
2. 把Q群单独拉出来用，如下图：
...
2. 权限管理指令
    ...
    #关闭指令权限限制(`limit`)
3. 额外指令
    #jm（下载禁漫的指令）(`limit`)[2.1.0版本后迁移到了插件中，指令无法调用了]
    #图片映射表的键（具体见下文，作用是批量下载图片）[2.0.0后插件也能够调用了]
4. 远程控制指令
    #中控截图(`limit`)
    #运行时间(`limit`)
    #系统资源监控(`limit`)
...
### 从P站插画排行榜爬取
插件文件：`pixiv_crawler.py`
作用：从P站插画排行榜中爬取图片，每日、每周、每月、新人以及是否r18
限制：必须自己挂VPN，部分图片需要登陆
cookie文件：`用户设置/web_data/pixiv_cookies.txt`
...
- 学习的东西拿来练手了，没想过居然能直更新到2.0，现在也算是一个Agent了，哈哈哈哈
...
## 2.1.0 指令内化，P站排行插画爬取🛡️

**📌 QQ版本支持**
- 🎯 支持 QQ 9.9.25-42941 (64位)

**🔄 架构与功能迁移**
- 🔧 将 `jm` 指令完整迁移至插件系统，实现模块化管理
- 🖼️ 新增 **Pixiv（P站）排行榜插画爬取与发送功能**，获取每日/每周/每月/新人插画(自己上cookies发r18)

**🎨 兼容性重大提升**
- ✅ **全面兼容QQ各类主题**，无需再手动设置“极简”主题（经穷尽测试验证）
- 🚫 移除旧版强制主题设置要求，使用体验更自由便捷

**🔐 安全与权限修复**
- 🛡️ **重写指令鉴权核心逻辑**，彻底修复远程控制指令可能被非管理员调用的安全漏洞
- 📊 权限验证更精准严格，确保系统指令安全可控


## 紧急事项
直接接入langchain框架或许会更好，
3.0.0、4.0.0、5.0.0这些目标都能一次性实现，虽然我确实想搓轮子，但是已经不是这个搓轮子的时代了
...
- [x] 使用开源项目接入P站爬取图片(草了，没有这种项目，我手搓了个排行榜的)
- [ ] 初始化所有配置(清空用户数据)
- [ ] 优化添加删除绑定配置UI流程
- [ ] 开启自动回复按钮多次点击或卡死
- [x] 给QQ收拾烂摊，定时重启QQ窗口释放内存，资源消耗比我还大。（2026它牛大了，做了优化升级，我也不得不重写UI解析了）
- [ ] 定时清空聊天记录
- [ ] 发送电脑当前状态（psutil库）
- [ ] 需要再次深入研究窗口置顶问题（经常性的出现初始化窗口时没有绑定）
- [ ] 2.0.0版本存在调用`#聊天记录`会有回调(原因是设置了关键词回复)
